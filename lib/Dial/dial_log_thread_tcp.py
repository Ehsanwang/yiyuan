...
class LogThread(Thread):
    ...
    def init_log_dir(self, info):
        ...
        init_df_column = ['runtimes', 'dial_connect_time', 'dial_fail_times', 'tcp_udp_connect_time',
                          'tcp_udp_connect_fail_times', 'file_compare_fail_times', 'ip_error_times',
                          'recv_file_time', 'tcp_udp_disconnect_time', 'tcp_udp_disconnect_fail_times',
                          'dial_disconnect_time', 'dial_disconnect_fail_times', 'ping_package_loss_rate',
                          'ip_address', 'qtemp_list', 'dial_disconnect_success_times', 'cefs_erase_times', 'usrdata_erase_times']
    ...
    def write_result_log(self, runtimes):
        ...
        result_width_standard_1 = {0: ['local_time', 25], 1: ['runtimes', 8],
                                   ...
                                   4: ['tcp_udp_connect_time', 20], 5: ['tcp_udp_connect_fail_times', 26],
                                   6: ['file_compare_fail_times', 23], 7: ['ip_error_times', 14],
                                   8: ['recv_file_time', 14],
                                   9: ['tcp_udp_disconnect_time', 23], 10: ['tcp_udp_disconnect_fail_times', 29],
                                   11: ['dial_disconnect_time', 20], 12: ['dial_disconnect_fail_times', 26],
                                   13: ['ping_package_loss_rate', 22], 14: ['ip_address', 23],
                                   15: ['qtemp_list', 50], 16: ['cefs_erase_times', 10], 17: ['usrdata_erase_times', 10]}
        result_width_standard_0 = {0: ['local_time', 25], 1: ['runtimes', 8], 3: ['file_compare_fail_times', 23],
                                   4: ['recv_file_time', 14], 5: ['ping_package_loss_rate', 22], 6: ['ip_address', 23],
                                   7: ['qtemp_list', 50], 8: ['cefs_erase_times', 10], 9: ['usrdata_erase_times', 10]}
        ...
        tcp_udp_connect_time = '' if pd.isna(self.df.loc[runtimes, 'tcp_udp_connect_time']) else round(float(self.df.loc[runtimes, 'tcp_udp_connect_time']), 2)
        tcp_udp_connect_fail_times = int(self.df['tcp_udp_connect_fail_times'].sum())
        file_compare_fail_times = int(self.df['file_compare_fail_times'].sum())
        ip_error_times = int(self.df['ip_error_times'].sum())
        recv_file_time = '' if pd.isna(self.df.loc[runtimes, 'recv_file_time']) else round(float(self.df.loc[runtimes, 'recv_file_time']), 2)
        tcp_udp_disconnect_time = '' if pd.isna(self.df.loc[runtimes, 'tcp_udp_disconnect_time']) else round(float(self.df.loc[runtimes, 'tcp_udp_disconnect_time']), 2)
        tcp_udp_disconnect_fail_times = int(self.df['tcp_udp_disconnect_fail_times'].sum())
        ...
        result_list_1 = [local_time, runtimes, dial_connect_time, dial_connect_fail_times,
                         tcp_udp_connect_time, tcp_udp_connect_fail_times,
                         file_compare_fail_times, ip_error_times, recv_file_time,
                         tcp_udp_disconnect_time, tcp_udp_disconnect_fail_times,
        ...
        result_list_2 = [local_time, runtimes, file_compare_fail_times, recv_file_time, ping_package_loss_rate,
                         ip_address, qtemp_list, cefs_erase_times, usrdata_erase_times]
        ...
        # -----------------------------------------------------------------------------
        # TCP连接时间      tcp_udp_connect_time
        # TCP失败次数      tcp_udp_connect_fail_times
        # TCP成功率        tcp_udp_connect_success_times/(tcp_udp_connect_success_times + tcp_udp_connect_fail_times)
        # -----------------------------------------------------------------------------
        # TCP断开连接时间  tcp_udp_disconnect_time
        # TCP断开失败次数  tcp_udp_disconnect_fail_times
        # TCP断开成功率tcp_udp_disconnect_success_times/(tcp_udp_disconnect_success_times + tcp_udp_disconnect_fail_times
        # -----------------------------------------------------------------------------
    ...
    def end_script(self, script_start_time, connection_type, runtimes):
        ...
        if self.connect_mode == 1:
            ...
            # TCP连接参数
            tcp_udp_connect_time_avg = np.round(np.mean(self.df['tcp_udp_connect_time'].dropna()), 2)  # tcp_udp连接平均时间
            tcp_udp_connect_fail_times = int(self.df['tcp_udp_connect_fail_times'].sum())  # tcp_udp连接失败次数
            tcp_udp_success_rate = '{:.2f}%'.format(self.df['tcp_udp_connect_success_times'].sum() * 100 / (self.df['tcp_udp_connect_fail_times'].sum() + self.df['tcp_udp_connect_success_times'].sum()))  # tcp_upd连接成功率
            # TCP断开连接参数
            tcp_udp_disconnect_time_avg = np.round(np.mean(self.df['tcp_udp_disconnect_time'].dropna()), 2)  # tcp_udp断开连接平均时间
            tcp_udp_disconnect_fail_times = int(self.df['tcp_udp_disconnect_fail_times'].sum())  # tcp_udp连接失败次数
            tcp_udp_disconnect_success_rate = '{:.2f}%'.format(self.df['tcp_udp_disconnect_success_times'].sum() * 100 / (self.df['tcp_udp_disconnect_fail_times'].sum() + self.df['tcp_udp_disconnect_success_times'].sum()))  # tcp_upd断开连接成功率
            # 拨号断开连接参数
            ...
            file_recv_time_avg = np.round(np.mean(self.df['recv_file_time'].dropna()), 2)  # 拨号断开连接平均时间
            ip_error_times = int(self.df['ip_error_times'].sum())
            result = '\n[{}]-[{}]\n'.format(script_start_time_format, script_end_time_format) + \
                     ...
                     '{}连接平均时间(tcp_udp_connect_time_avg)：{}秒\n'.format(connection_type, tcp_udp_connect_time_avg) + \
                     '{}连接失败次数(tcp_udp_connect_fail_times)：{}次\n'.format(connection_type, tcp_udp_connect_fail_times) + \
                     '{}连接成功率(tcp_udp_success_rate): {}\n'.format(connection_type, tcp_udp_success_rate) + \
                     '{}断开连接平均时间(tcp_udp_disconnect_time_avg)：{}秒\n'.format(connection_type, tcp_udp_disconnect_time_avg) + \
                     '{}断开连接失败次数(tcp_udp_disconnect_fail_times)：{}次\n'.format(connection_type, tcp_udp_disconnect_fail_times) + \
                     '{}断开连接成功率(tcp_udp_disconnect_success_rate): {}\n'.format(connection_type, tcp_udp_disconnect_success_rate) + \
                     ...
                     '文件接收平均时间(file_recv_time_avg)：{}秒\n'.format(file_recv_time_avg) + \
