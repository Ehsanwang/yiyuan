...
class RWManager:
    def __init__(self, main_queue, route_queue, uart_queue, log_queue, uart_port, version, imei, rwfile_path, rwfile,
    ...
    def rw_init(self):
        ...
        if main_queue_data is False:
            ...
            exit()
        # 6. 识别adb devices
        main_queue_data = self.route_queue_put('Process', 'check_adb_devices_connect', self.runtimes)
        if main_queue_data is False:
            print('adb devices异常，退出脚本')
            exit()
        # # 7. adb push 文件到/bin下
        # main_queue_data = self.route_queue_put('Process', 'adb_push_rw_file', self.rwfile_path, self.rwfile, self.runtimes)
        # if main_queue_data is False:
        #     print('adb push异常，退出脚本')
        #     exit()
        # 8. 关闭AT口
