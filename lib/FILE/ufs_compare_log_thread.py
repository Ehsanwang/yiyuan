...
class LogThread(Thread):
    ...
    def init_log_dir(self, info):
        ...
        init_df_column = ['local_time', 'runtimes', 'upl_file_time',
                          'dwl_file_time', 'upl_fail_times',
                          'dwl_fail_times', 'open_fail_times', 'del_fail_times', 'qftest_record', 'cefs_erase_times',
                          'usrdata_erase_times']
    ...
    def write_result_log(self, runtimes):
        ...
        result_width_standard = {0: ['local_time', 25], 1: ['runtimes', 8],
                                 2: ['upl_file_time', 13], 3: ['dwl_file_time', 13],
                                 4: ['upl_fail_times', 14], 5: ['dwl_fail_times', 14],
                                 6: ['open_fail_times', 15], 7: ['del_fail_times', 14], 8: ['cefs_erase_times', 10],
        ...
        upl_file_time = '' if pd.isna(self.df.loc[runtimes, 'upl_file_time']) else round(float(self.df.loc[runtimes, 'upl_file_time']), 2)
        dwl_file_time = '' if pd.isna(self.df.loc[runtimes, 'dwl_file_time']) else round(float(self.df.loc[runtimes, 'dwl_file_time']), 2)
        upl_fail_times = int(self.df['upl_fail_times'].sum())
        dwl_fail_times = int(self.df['dwl_fail_times'].sum())
        open_fail_times = int(self.df['open_fail_times'].sum())
        del_fail_times = int(self.df['del_fail_times'].sum())
        ...
        result_list = [local_time, runtimes, upl_file_time, dwl_file_time,
                       upl_fail_times, dwl_fail_times, open_fail_times, del_fail_times, cefs_erase_times, usrdata_erase_times]
    ...
    def end_script(self, script_start_time, runtimes):
        ...
        upl_file_time_avg = np.round(np.mean(self.df['upl_file_time'].dropna()), 2)
        dwl_file_time_avg = np.round(np.mean(self.df['dwl_file_time'].dropna()), 2)
        upl_fail_times = int(self.df['upl_fail_times'].sum())
        dwl_fail_times = int(self.df['dwl_fail_times'].sum())
        open_fail_times = int(self.df['open_fail_times'].sum())
        del_fail_times = int(self.df['del_fail_times'].sum())
        ...
        result = '\n[{}]-[{}]\n'.format(script_start_time_format, script_end_time_format) + \
                 ...
                 '上传文件平均时间(upl_file_time_avg)：{}秒\n'.format(upl_file_time_avg) + \
                 '下载文件平均时间(dwl_file_time_avg)：{}秒\n'.format(dwl_file_time_avg) + \
                 '上传文件失败次数(upl_fail_times)：{}次\n'.format(upl_fail_times) + \
                 '下载文件失败次数(dwl_fail_times)：{}次\n'.format(dwl_fail_times) + \
                 '打开文件失败次数(open_fail_times)：{}次\n'.format(open_fail_times) + \
                 '删除文件失败次数(del_fail_times)：{}次\n'.format(del_fail_times) + \
