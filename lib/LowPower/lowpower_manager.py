...
class LowPower:
    def __init__(self, main_queue, route_queue, uart_queue, log_queue, uart_port, evb, version, imei, vbat, is_debug, runtimes):
    ...
    def low_power_vbat(self):
        ...
        time.sleep(10)
        if self.is_debug:
            main_queue_data = self.route_queue_put('Uart', 'lowpower_debug', 'oelinux123', self.runtimes)
            if main_queue_data is False:
                print('初始化debug开启慢时钟功能失败')
                return False
    ...
    def lp_init(self):
        ...
        # 7. 关闭AT口
        if self.is_debug:   # 需要进入debug输入指令开启慢时钟
            main_queue_data = self.route_queue_put('Uart', 'lowpower_debug', 'oelinux123', self.runtimes)
            if main_queue_data is False:
                print('初始化debug开启慢时钟功能失败')
                exit()
