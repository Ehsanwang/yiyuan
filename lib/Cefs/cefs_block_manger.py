...
class CefsBlockManger:
    def __init__(self, main_queue, route_queue, uart_queue, at_queue, log_queue, at_port, uart_port, version, imei,
        ...
        self.block_restore_count = block_restore_count
        self.total_restore_count = total_restore_count
    ...
    def block_init(self):
        ...
        if main_queue_data is False:
            return False
        time.sleep(12)
        if self.runtimes == 0:  # 设置还原次数上限 及备份
            main_queue_data = self.route_queue_put('AT', 'set_blocklimit', self.block_restore_count,
                                                   self.total_restore_count, self.runtimes)
            if main_queue_data is False:
                return False
        # 5. 关闭AT口
    ...
    def blockinit(self):
        ...
        time.sleep(6)
        # 2.设置主动还原
        main_queue_data = self.route_queue_put('AT', 'set_restore', self.runtimes)
        if main_queue_data is False:
            return False
        time.sleep(2)
        # 3. 断电后上电
