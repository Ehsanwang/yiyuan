...
class LogThread(Thread):
    def __init__(self, version, info, log_queue, main_queue):
        ...
        self.cefsblock_log_handle = open('CEFSBlockLog-{}.txt'.format(self.version), "a+", encoding='utf-8', buffering=1)

        self.handles = ['self.at_log_handle', 'self.dos_log_handle', 'self.debug_log_handle',
                        'self.result_log_handle', 'self.network_log_handle', 'self.cefsblock_log_handle']
    ...
    def cefsblock_log(self, log_queue_data):
        self.cefsblock_log_handle.write('{}{}'.format(log_queue_data, '' if log_queue_data.endswith('\n') else '\n'))
    ...
    def init_log_dir(self, info):
        ...
        init_df_column = ['runtimes_start_timestamp', 'runtimes', 'qftest_record',
    ...
    def write_result_log(self, runtimes):
        ...
        """
        # 用作写入result_log时宽度设置
        result_width_standard_1 = {0: ['local_time', 25], 1: ['runtimes', 10], 2: ['cefs_erasetimes', 10],
                                   3: ['usrdata_erasetimes', 10]}
        ...
        result_list = [local_time, runtimes, cefs_erase_times, usrdata_erase_times]
