...
class LogThread(Thread):
    def __init__(self, version, info, log_queue, main_queue):
        ...
        self.ab_bootfail_log_handle = open('AB_BOOTFAIL_LOG-{}.txt'.format(self.version), "a+", encoding='utf-8',
                                           buffering=1)

        self.handles = ['self.at_log_handle', 'self.dos_log_handle', 'self.debug_log_handle',
                        'self.result_log_handle', 'self.network_log_handle', 'self.ab_bootfail_log_handle']
    ...
    def ab_bootfail_log(self, log_queue_data):
        self.ab_bootfail_log_handle.write('{}{}'.format(log_queue_data, '' if log_queue_data.endswith('\n') else '\n'))
    ...
    def init_log_dir(self, info):
        ...
        init_df_column = ['runtimes_start_timestamp', 'runtimes', 'abboot_fail_times', 'qftest_record',
    ...
    def write_result_log(self, runtimes):
        ...
        """
        # 用作写入result_log时宽度设置
        result_width_standard_1 = {0: ['local_time', 25], 1: ['runtimes', 10], 2: ['abboot_fail_times', 20],
                                   3: ['cefs_erase_times', 10], 4: ['usrdata_erase_times', 10]}
        ...
        abboot_fail_times = int(self.df['abboot_fail_times'].sum())
        ...
        result_list = [local_time, runtimes, abboot_fail_times, cefs_erase_times, usrdata_erase_times]
    ...
    def end_script(self, script_start_time, runtimes):
        ...
        abboot_fail_times = int(self.df['abboot_fail_times'].sum())  # CFUN切换失败次数
        result = '\n[{}]-[{}]\n'.format(script_start_time_format, script_end_time_format) + \
                 '共运行{}H/{}次\n'.format(round((time.time() - script_start_time) / 3600, 2), runtimes) + \
                 'abboot切换失败次数(abboot_fail_times)：{}次\n'.format(abboot_fail_times) + \
