...
class LogThread(Thread):
    def __init__(self, version, info, adb_or_at, log_queue, main_queue):
        super().__init__()
        ...
        self.memory_log_handle = open('MemoryLog-{}.txt'.format(self.version), "a+", encoding='utf-8', buffering=1)

        self.handles = ['self.at_log_handle', 'self.dos_log_handle', 'self.debug_log_handle',
                        'self.result_log_handle', 'self.network_log_handle', 'self.memory_log_handle']
    ...
    def memory_log(self, log_queue_data):
        self.memory_log_handle.write('{}{}'.format(log_queue_data, '' if log_queue_data.endswith('\n') else '\n'))
    ...
    def init_log_dir(self, info):
        ...
        init_df_column = ['local_time', 'runtimes', 'fota_a_b_start_timestamp', 'fota_a_b_end_timestamp',
                          ...
                          'fota_ab_start_timestamp', 'fota_ab_end_timestamp', 'cefs_erase_times', 'usrdata_erase_times']
    ...
    def write_result_log(self, runtimes):
        ...
        #fota_ab_start_timestamp  ab 系统开始备份时间
        #fota_ab_end_timestamp   ab系统备份结束时间
        # 统计参数
        # fota_a_b_fail_times a_b升级失败的次数
        ...
        result_width_standard = {0: ['local_time', 25], 1: ['runtimes', 8],
                                 ...
                                 8: ['net_fail_times', 14], 9: ['fota_ab_backup_time', 21], 10: ['cefs_erase_times', 10],
        ...
        fota_ab_backup_time = '' if pd.isna(self.df.loc[runtimes, 'fota_ab_end_timestamp']) or pd.isna(self.df.loc[runtimes, 'fota_ab_start_timestamp']) else np.round(self.df.loc[runtimes, 'fota_ab_end_timestamp'] - self.df.loc[runtimes, 'fota_ab_start_timestamp'], 2)
        ...
        result_list = [local_time, runtimes, fota_a_b_fail_times, fota_a_b_upgrade_time,
                       ...
                       fota_a_b_version_fail_times, fota_b_a_version_fail_times, net_fail_times, fota_ab_backup_time,
    ...
    def end_script(self, script_start_time, runtimes):
        ...
        fota_ab_backtime_frame = self.df.loc[(self.df.fota_ab_start_timestamp.notnull()) & (self.df.fota_ab_end_timestamp.notnull()), ['fota_ab_start_timestamp', 'fota_ab_end_timestamp']]
        fota_a_b_backaverage_time = np.round( np.mean(fota_ab_backtime_frame['fota_ab_end_timestamp'] - fota_ab_backtime_frame['fota_ab_start_timestamp']), 2)
        fota_b_a_fail_times = int(self.df['fota_b_a_fail_times'].sum())
        ...
        result = '\n[{}]-[{}]\n'.format(script_start_time_format, script_end_time_format) + \
                 ...
                 'AB系统同步平均时间(fota_a_b_backaverage_time)：{}\n'.format(fota_a_b_backaverage_time) + \
