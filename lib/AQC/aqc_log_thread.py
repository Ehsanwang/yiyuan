...
class LogThread(Thread):
    ...
    def init_log_dir(self, info):
        ...
        init_df_column = ['runtimes_start_timestamp', 'runtimes', 'rmnet_Fail_times', 'IP_fail_times', 'rmnet_success_rate',
                          'ip_succecss_rate', 'ip_address', 'cefs_erase_times', 'usrdata_erase_times']
    ...
    def write_result_log(self, runtimes):
        ...
        """
        # 用作写入result_log时宽度设置
        result_width_standard_1 = {0: ['local_time', 25], 1: ['runtimes', 10], 2: ['rmnet_Fail_times', 20],
                                   3: ['IP_fail_times', 20], 4: ['rmnet_success_rate', 20], 5: ['ip_succecss_rate', 20],
                                   6: ['ip_address', 20], 7: ['cefs_erase_times', 10], 8: ['usrdata_erase_times', 10]}
        ...
        rmnet_Fail_times = int(self.df['rmnet_Fail_times'].sum())
        IP_fail_times = int(self.df['IP_fail_times'].sum())
        rmnet_success_rate = '{:.2f}%'.format((runtimes - rmnet_Fail_times) * 100 / runtimes)  # rmnet_data0加载成功率
        ip_succecss_rate = '{:.2f}%'.format((runtimes - IP_fail_times) * 100 / runtimes)  # IP地址识别成功率
        ...
        result_list = [local_time, runtimes, rmnet_Fail_times, IP_fail_times, rmnet_success_rate, ip_succecss_rate, ip_address,
    ...
    def end_script(self, script_start_time, runtimes):
        ...
        rmnet_Fail_times = int(self.df['rmnet_Fail_times'].sum())  # rmnet0识别失败次数
        IP_fail_times = int(self.df['IP_fail_times'].sum())  # IP地址识别失败次数
        rmnet_success_rate = '{:.2f}%'.format((runtimes - rmnet_Fail_times) * 100 / runtimes)  # rmnet_data0加载成功率
        ip_succecss_rate = '{:.2f}%'.format((runtimes - IP_fail_times) * 100 / runtimes)  # IP地址识别成功率
        ...
        result = '\n[{}]-[{}]\n'.format(script_start_time_format, script_end_time_format) + \
                 ...
                 'rmnet_data0加载失败次数(rmnet_Fail_times)：{}次\n'.format(rmnet_Fail_times) + \
                 'rmnet_data0加载成功率(rmnet_rate)：{}次\n'.format(rmnet_success_rate) + \
                 'IP地址识别失败次数(IP_fail_times)：{}次\n'.format(IP_fail_times) + \
                 'IP地址识别成功率(ip_rate)：{}次\n'.format(ip_succecss_rate) + \
