...
class LogThread(Thread):
    ...
    def end_script(self, script_start_time, runtimes):
        ...
        write_size = round(float(np.sum(self.df['write_size'].dropna())), 3)
    ...
    def write_result_log(self, runtimes):
        ...
        result_width_standard = {0: ['local_time', 25], 1: ['runtimes', 8], 2: ['write_size', 10], 3: ['write_size_total', 16],
                                 4: ['restore_times', 13]}
        ...
        write_size = 0 if pd.isna(self.df.loc[runtimes, 'write_size']) else round(self.df.loc[runtimes, 'write_size'], 3)
        write_size_total = round(self.df['write_size'].sum(), 3)
        restore_times = int(np.max(self.df['restore_times']))

        result_list = [local_time, runtimes, write_size, write_size_total, restore_times]
