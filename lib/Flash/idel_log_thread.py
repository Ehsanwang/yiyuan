...
class LogThread(Thread):
    ...
    def write_result_log(self, runtimes):
        ...
        """
        # 用作写入result_log时宽度设置
        result_width_standard_1 = {0: ['local_time', 25], 1: ['runtimes', 10], 2: ['cefs_erasetimes', 10], 3: ['usrdata_erasetimes', 10]}
        ...
        cefs_erasetimes = '' if pd.isna(self.df.loc[runtimes, 'cefs_erase_times']) else int(self.df.loc[runtimes, 'cefs_erase_times'])
        usrdata_erasetimes = '' if pd.isna(self.df.loc[runtimes, 'usrdata_erase_times']) else int(self.df.loc[runtimes, 'usrdata_erase_times'])
        # if runtimes == 1:
        ...
        result_list = [local_time, runtimes, cefs_erasetimes, usrdata_erasetimes]
    ...
    def end_script(self, script_start_time, runtimes):
        ...
        # cefs_erase_maxtimes = cefs_erasetimes_list[0]
        #
        ...
        # usrdata_erase_maxtimes = ufs_erasetimes_list[0]
