...
class SmsManager:
    ...
    def sms_init(self):
        ...
        if main_queue_data is False:
            return False
        # 检测驻网  # AT+CMSP=17, 71, 0, 0 不上报 SMS DOWN可能会上报CMS ERROR: 310
        main_queue_data = self.route_queue_put('AT', 'check_network', False, self.runtimes)
        if main_queue_data is False:
            return False
        # 6. sms AT初始化
        main_queue_data = self.route_queue_put('AT', 'sms_init', self.pdu_or_text, self.cscs, self.runtimes)
        if main_queue_data is False:
            return False
        # 7. 关闭AT口
