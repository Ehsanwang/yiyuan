...
class MbnCheck:
    def __init__(self, main_queue, route_queue, uart_queue, log_queue, uart_port, evb, version, imei, is_calibration,
                 VBAT, is_5g_m2_evb, mbn_list, runtimes):
    ...
    def mbncheck_normal_exception_init(self):
        # 关口防止端口变化
    ...
    def qfirehose_upgrade_vbat(self, packagename, mode, runtimes):
        if self.is_5g_m2_evb:
            self.route_queue_put('Uart', 'set_rts_false')
            self.log_queue.put(['at_log', "[{}] Set RTS False".format(datetime.datetime.now())])
            ...
            main_queue_data = self.route_queue_put('Process', 'qfirehose_upgrade', packagename, False, mode, runtimes)
            if main_queue_data is False:
    ...
    def random_vbat_check_mbn(self):
        """
        升级成功后随机断电重启检查mbn
        """
        self.log_queue.put(['at_log', "[{}] 升级完成重启后进行连续10次随机断电查询mbn".format(datetime.datetime.now())])
        for i in range(1, 11):
            self.log_queue.put(['at_log', "[{}] 当前进行第{}次随机断电查询mbn".format(datetime.datetime.now(), i)])
            None if self.runtimes == 0 else self.route_queue_put('AT', 'close', self.runtimes)
            vbat_value = random.randint(2, 20)
            self.log_queue.put(['at_log', "[{}] 等待{}秒后断电重启".format(datetime.datetime.now(), vbat_value)])
            time.sleep(vbat_value)
            self.vbat()
            # 初始化检测USB驱动
            main_queue_data = self.route_queue_put('Process', 'check_usb_driver', True, self.runtimes)
            if main_queue_data is False:
            ...
            if main_queue_data is False:
                continue
            time.sleep(20)
            main_queue_data = self.route_queue_put('AT', 'check_mbn_lost', self.mbn_list, self.runtimes)
            if main_queue_data is False:
                continue
