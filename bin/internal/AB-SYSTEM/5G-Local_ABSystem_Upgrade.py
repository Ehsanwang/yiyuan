...
3. 准备DFOTA升级的版本号
4. 需要打开模块adb功能，AT+QADBKEY?查询并解锁，AT+QCFG=""USBCFG"",0x2C7C,0x0800,1,1,1,1,1,1打开ADB功能
...
3. 重复1-2。
"""
# SG520TMDAR01A01M4G_BETA_20220128A V03
# SG520TMDAR01A02M4G  V01
...
after_upgrade_revision = "SG520TMDAR01A01M4G_BETA_20220128A"  # 设置升级后的Revision版本号，填写ATI+CSUB查询的Revision部分
after_upgrade_sub_edition = "V03"  # 设置升级后的SubEdition版本号，填写ATI+CSUB查询的SubEdition部分
a_b_package_path = r'C:\Users\Jaime.Sun\Desktop\CFB\SG520TMDA_A02_TO_BETA.zip'  # a到b版本的升级包路径，精确到包名，路径是本地电脑路径
b_a_package_path = r'C:\Users\Jaime.Sun\Desktop\CFB\SG520TMDA_BETA_TO_A02.zip'  # b到a版本的升级包路径，精确到包名，路径是本地电脑路径
a_b_packagename = 'SG520TMDA_A02_TO_BETA.zip'  # a到b的差分包名称，用于后面的指令命名
b_a_packagename = 'SG520TMDA_BETA_TO_A02.zip'  # b到a的差分包名称，用于后面的指令命名
...
# ======================================================================================================================
# 可选参数
runtimes_temp = 3  # 设置脚本运行次数，默认0是不限次数，如果设置1000则脚本运行1000个runtimes后自动结束
...
dfota_info = 'Local_DFOTA-{}-{}-{}'.format(info, '升级断电' if VBAT else '正常升级',
    ...
    for mode in ('forward', 'backward'):  # forward代表正向升级，backward代表逆向升级
        ...
        if is_ufs:
            while True:
                ...
                # 7.差分包命名
                if mode == 'forward':
                    main_queue_data = route_queue_put('AT', 'set_package_name', abpackagename, runtimes)
                    if main_queue_data is False:
                        dfota_manager.dfota_ab_exception_init()
                        continue
                if mode == 'backward':
                    main_queue_data = route_queue_put('AT', 'set_package_name', bapackagename, runtimes)
                    if main_queue_data is False:
                        dfota_manager.dfota_ab_exception_init()
                        continue
