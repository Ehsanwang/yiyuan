...
abboot_info = '随机破坏AB分区--{}分区'.format(Partition_Type)
# ======================================================================================================================
# 定义queue并启动线程
main_queue = Queue()
route_queue = Queue()
...
while True:
    ...
    for mode in ('forward', 'backward'):  # forward代表正向升级，backward代表逆向升级
        ...
        if mode == 'forward':
            log_queue.put(['at_log', '[{}] 开始进行A分区boot分区破坏'.format(datetime.datetime.now())])

        if mode == 'backward':
            log_queue.put(['at_log', '[{}] 开始进行B分区boot分区破坏'.format(datetime.datetime.now())])
        ...
        while True:
            ...
            # 擦除A分区BOOT分区
            main_queue_data = route_queue_put('AT', 'break_ab_boot', mode, Partition_Type, Partition_rangea, Partition_rangeb, runtimes)
            if main_queue_data is False:
                ab_boot_manager.ab_boot_init()
                continue
            ...
            # 检查擦除后分区
            main_queue_data = route_queue_put('AT', 'check_abboot', mode, runtimes)
            if main_queue_data is False:
                ab_boot_manager.ab_boot_init()
                continue
