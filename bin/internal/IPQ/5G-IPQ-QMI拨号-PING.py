...
脚本逻辑
PCIE-QMI短拨+ping网
1、模块设置at+qcfg="data_interface",1,0
2、QMI拨号，拨号成功后ping网
3、QMI拨号断开
4、循环步骤2~3

AT引导的PCIE+开关机+拨号
1、模块配置AT+qcfg="data_interface",1，0
2、模块重启，检测PCIE枚举是否正常，
3、QMI拨号，ping网
...
restart_flag = True   # False: QMI短拨+PING   True: 开关机+QMI+PING
# ======================================================================================================================
# 辅助参数
...
ipq_info = 'IPQ-开关机-QMI-PING' if restart_flag else 'IPQ-QMI短拨-PING'   # 脚本详情字符串
...
ipq_manager.ipq_pcie_qmi_init()  # IPQ初始化
# ======================================================================================================================
# 主流程
while True:
    # 脚本自动停止判断
    ...
    if restart_flag:
        # 开关机+QMI+PING
        qmi_ping_restart_status = ipq_manager.ipq_qmi_ping_restart()
        if qmi_ping_restart_status is False:
            ipq_manager.ipq_qmi_re_init()
            continue
    else:
        # qmi短拨+PING
        qmi_ping_norestart_status = ipq_manager.ipq_qmi_ping_norestart()
        if qmi_ping_norestart_status is False:
            ipq_manager.ipq_qmi_re_init(False)
            continue
