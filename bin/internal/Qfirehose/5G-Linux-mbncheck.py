...
from Mbncheck_manager import MbnCheck
...
A_B_package_name = '/home/cris/standard/RG500QEAAAR11A05M4G_01.001V01.01.001V01/RG500QEAAAR11A05M4G_01.001V01.01.001V01'   # 不同版本填写升级后标准版本包路径名称
B_A_package_name = '/home/cris/standard/RG500QEAAAR11A05M4G_01.001V02.01.001V02/RG500QEAAAR11A05M4G_01.001V02.01.001V02'   # 若是不同版本升级，则此处填写升级前标准版本包名称 ,否则不填
mbn_list = ['Volte_OpenMkt-Commercial-CMCC', 'ROW_Generic_3GPP_PTCRB_GCF', 'FirstNet', 'Rogers_Canada', 'Bell_Canada',
            'Telus_Canada', 'Commercial-Sprint', 'Commercial-TMO', 'VoLTE-ATT', 'CDMAless-Verizon', 'Telia_Sweden',
            'TIM_Italy_Commercial', 'France-Commercial-Orange', 'Commercial-DT-VOLTE', 'Germany-VoLTE-Vodafone',
            'UK-VoLTE-Vodafone', 'Commercial-EE', 'Optus_Australia_Commercial', 'Telstra_Australia_Commercial',
            'Commercial-LGU', 'Commercial-KT', 'Commercial-SKT', 'Commercial-Reliance', 'Commercial-SBM',
            'Commercial-KDDI', 'Commercial-DCM', 'VoLTE-CU', 'VoLTE_OPNMKT_CT', 'ROW_Commercial']   # 当前测试版本mbn列表,AT+QMBNCFG= "list" 查询后填入
...
MbnCheck_info = 'Mbncheck'  # 脚本详情字符串
...
log_thread = LogThread(version, MbnCheck_info, log_queue, main_queue, is_same_upgrade)
...
MbnCheck_manager = MbnCheck(main_queue, route_queue, uart_queue, log_queue, uart_port, evb, version, imei,
                              is_calibration, VBAT, is_5g_m2_evb, mbn_list, runtimes)
log_queue.put(['at_log', '{}initialize{}'.format('=' * 35, '=' * 35)])
print("\rinitialize", end="")
MbnCheck_manager.mbncheck_init()
# ======================================================================================================================
# 主流程
while True:
    # 脚本自动停止判断
    ...
    MbnCheck_manager.runtimes = runtimes

    # 打开AT口
    main_queue_data = route_queue_put('AT', 'open', runtimes)
    if main_queue_data is False:
        MbnCheck_manager.mbncheck_normal_exception_init()
        continue
    ...
    if main_queue_data is False:
        MbnCheck_manager.mbncheck_normal_exception_init()
        continue
    # 进行Qfirehose升级，如果是相同版本升级，则只升级一次,否则进行正反向升级，先升级工厂再升级标准
    if is_same_upgrade:     # 同版本升级
        ...
        for i in range(1, 5):
            if i == 4:
                ...
                pause()
            main_queue_data = route_queue_put('Process', 'qfirehose_upgrade', A_B_package_name, VBAT, 'forward', runtimes)
            if main_queue_data is False:
                MbnCheck_manager.upgrade_fail_init(runtimes)
                continue
            if VBAT:
                qfirehose_status = MbnCheck_manager.qfirehose_upgrade_vbat(A_B_package_name, 'forward', runtimes)
            ...
            # 升级完成后再次重启
            MbnCheck_manager.random_vbat_check_mbn()
            ...
            if mode == 'forward':
                for i in range(1, 5):
                    ...
                    if is_factory:
                        ...
                        main_queue_data = route_queue_put('Process', 'qfirehose_upgrade', A_B_factory_package_name,
                                                          VBAT, mode, runtimes)
                        if main_queue_data is False:
                            MbnCheck_manager.upgrade_fail_init(runtimes)
                            continue
                        if VBAT:
                            qfirehose_status = MbnCheck_manager.qfirehose_upgrade_vbat(A_B_factory_package_name, mode,
                                                                                        runtimes)
                            if qfirehose_status is False:
                                MbnCheck_manager.upgrade_fail_init(runtimes)
                                continue
                    ...
                    # 升级标准版本
                    main_queue_data = route_queue_put('Process', 'qfirehose_upgrade', A_B_package_name, VBAT, mode,
                                                      runtimes)
                    if main_queue_data is False:
                        MbnCheck_manager.upgrade_fail_init(runtimes)
                        continue
                    if VBAT:
                        qfirehose_status = MbnCheck_manager.qfirehose_upgrade_vbat(A_B_package_name, mode, runtimes)
                        if qfirehose_status is False:
                            MbnCheck_manager.upgrade_fail_init(runtimes)
                            continue
                    # 检测驱动加载
                ...
                for i in range(1, 5):
                    ...
                    if is_factory:
                        log_queue.put(
                            ['at_log', '[{}] 开始进行B-A工厂版本升级,当前版本为{}'.format(datetime.datetime.now(), after_version)])
                        main_queue_data = route_queue_put('Process', 'qfirehose_upgrade', B_A_factory_package_name,
                                                          VBAT, mode, runtimes)
                        if main_queue_data is False:
                            MbnCheck_manager.upgrade_fail_init(runtimes)
                            continue
                        if VBAT:
                            qfirehose_status = MbnCheck_manager.qfirehose_upgrade_vbat(B_A_factory_package_name, mode,
                                                                                        runtimes)
                            if qfirehose_status is False:
                                MbnCheck_manager.upgrade_fail_init(runtimes)
                                continue
                    ...
                    main_queue_data = route_queue_put('Process', 'qfirehose_upgrade', B_A_package_name, VBAT, mode,
                                                      runtimes)
                    if main_queue_data is False:
                        MbnCheck_manager.upgrade_fail_init(runtimes)
                        continue
                    if VBAT:
                        qfirehose_status = MbnCheck_manager.qfirehose_upgrade_vbat(B_A_package_name, mode, runtimes)
                        if qfirehose_status is False:
                            MbnCheck_manager.upgrade_fail_init(runtimes)
                            continue
                    # 检测驱动加载
