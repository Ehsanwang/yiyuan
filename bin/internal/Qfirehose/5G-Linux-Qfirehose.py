...
A_B_package_name = '/home/cris/standard/RG500QEAAAR11A05M4G_01.001V01.01.001V01/RG500QEAAAR11A05M4G_01.001V01.01.001V01'   # 若是同版本升级，只在此处填写当前标准版本包路径名称，不同版本填写升级后标准版本包路径名称
...
Qfirehose_info = 'Qfirehose-{}-{}'.format('相同版本升级' if is_same_upgrade else '不同版本升级', '升级断电' if VBAT else '正常升级')  # 脚本详情字符串
...
qfirehose_manager = Qfirehose(main_queue, route_queue, uart_queue, log_queue, uart_port, evb, version, imei,
...
while True:
    ...
    if is_same_upgrade:     # 同版本升级
        ...
        for i in range(1, 5):
            ...
            if main_queue_data is False:
                qfirehose_manager.upgrade_fail_init(runtimes)
                continue
            if VBAT:
                qfirehose_status = qfirehose_manager.qfirehose_upgrade_vbat(A_B_package_name, 'forward', runtimes)
            ...
            if mode == 'forward':
                for i in range(1, 5):
                    ...
                    if is_factory:
                        ...
                        main_queue_data = route_queue_put('Process', 'qfirehose_upgrade', A_B_factory_package_name, VBAT, mode, runtimes)
                        if main_queue_data is False:
                            qfirehose_manager.upgrade_fail_init(runtimes)
                            continue
                        if VBAT:
                            qfirehose_status = qfirehose_manager.qfirehose_upgrade_vbat(A_B_factory_package_name, mode, runtimes)
                            if qfirehose_status is False:
                                qfirehose_manager.upgrade_fail_init(runtimes)
                                continue
                        # 检测驱动加载
                    ...
                    # 升级标准版本
                    main_queue_data = route_queue_put('Process', 'qfirehose_upgrade', A_B_package_name, VBAT, mode, runtimes)
                    if main_queue_data is False:
                        qfirehose_manager.upgrade_fail_init(runtimes)
                        continue
                    if VBAT:
                        qfirehose_status = qfirehose_manager.qfirehose_upgrade_vbat(A_B_package_name, mode, runtimes)
                        if qfirehose_status is False:
                            qfirehose_manager.upgrade_fail_init(runtimes)
                            continue
                    # 检测驱动加载
                ...
                for i in range(1, 5):
                    ...
                    if is_factory:
                        ...
                        main_queue_data = route_queue_put('Process', 'qfirehose_upgrade', B_A_factory_package_name, VBAT, mode, runtimes)
                        if main_queue_data is False:
                            qfirehose_manager.upgrade_fail_init(runtimes)
                            continue
                        if VBAT:
                            qfirehose_status = qfirehose_manager.qfirehose_upgrade_vbat(B_A_factory_package_name, mode, runtimes)
                            if qfirehose_status is False:
                                qfirehose_manager.upgrade_fail_init(runtimes)
                                continue
                        # 检测驱动加载
                    ...
                    main_queue_data = route_queue_put('Process', 'qfirehose_upgrade', B_A_package_name, VBAT, mode, runtimes)
                    if main_queue_data is False:
                        qfirehose_manager.upgrade_fail_init(runtimes)
                        continue
                    if VBAT:
                        qfirehose_status = qfirehose_manager.qfirehose_upgrade_vbat(B_A_package_name, mode, runtimes)
                        if qfirehose_status is False:
                            qfirehose_manager.upgrade_fail_init(runtimes)
                            continue
                    # 检测驱动加载
