...
from policy_band_manager import PolicyBandManger
from route_thread import RouteThread
from at_thread import ATThread
from uart_thread import UartThread
from process_thread import ProcessThread
from policy_band_log_thread import LogThread

"""
脚本逻辑
1. at+qnwprefcfg="policy_band"    //检查policy_band 是否全0，全0则暂停脚本，保留现场
2. 如下指令若返回error则暂停脚本，保留现场
AT+QNVFR="/policyman/net_scan_time"
AT+QNVFR="/policyman/auto_learn_time"
AT+QNVFR="/policyman/device_config.xml"
AT+QNVFR="/policyman/global_defines.xml"
AT+QNVFR="/policyman/pre.xml"
AT+QNVFR="/policyman/lte_feature_restrictions.xml"
AT+QNVFR="/policyman/generic_band_restrictions.xml"
3. AT+QFTMMODE=1
4. AT+QFTMMODE?    //期望切换成功
5. 观察模块有无dump情况
6. AT+QFTMMODE=0
7. AT+QFTMMODE?   //期望切换成功
8. 观察模块有无dump情况
9. at+qnwprefcfg="policy_band"   //检查policy_band 是否全0，全0则暂停脚本，保留现场
10. 如下指令若返回error则暂停脚本，保留现场
AT+QNVFR="/policyman/net_scan_time"
AT+QNVFR="/policyman/auto_learn_time"
AT+QNVFR="/policyman/device_config.xml"
AT+QNVFR="/policyman/global_defines.xml"
...
policy_band_info = 'policy_band指令验证'
# ======================================================================================================================
# 定义queue并启动线程
main_queue = Queue()
route_queue = Queue()
...
log_thread = LogThread(version, policy_band_info, log_queue, False, main_queue)
...
policy_band_manger = PolicyBandManger(main_queue, route_queue, uart_queue, at_queue, log_queue, at_port, uart_port,
                                      version, imei, is_5g_m2_evb, runtimes)
log_queue.put(['at_log', '{}initialize{}'.format('=' * 35, '=' * 35)])
print("\rinitialize", end="")
cfun_init_status = policy_band_manger.policy_init()
...
while True:
    ...
    # 检查policy_band
    main_queue_data = route_queue_put('AT', 'check_policy_band', runtimes)
    if main_queue_data is False:
        policy_band_manger.policy_init()
        continue

    # 切换QFTMMODE
    main_queue_data = route_queue_put('AT', 'switch_qftmmode', runtimes)
    if main_queue_data is False:
        policy_band_manger.policy_init()
        continue
