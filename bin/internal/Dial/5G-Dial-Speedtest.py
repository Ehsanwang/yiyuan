...
modem_port = 'COM14'  # 定义MODEM端口号,用于检测拨号过程中网络状态信息
...
dial_mode = 'NDIS'  # 设置拨号方式 Windows: NDIS/MBIM/RGMII  LINUX: WWAN/MBIM/GOBINET/ECM
...
dial_manager = DialManager(main_queue, route_queue, uart_queue, log_queue, uart_port, evb, version, imei, dial_mode,
                           connection_mode, '', '', '', '', '', restart_mode, network_driver_name, is_5g_m2_evb, runtimes)
...
while True:
    ...
    if restart_mode is not None:  # 如果每个Runtimes需要进行开关机
        dial_init_speedtest_info = dial_manager.dial_poweroff_init()
        if dial_init_speedtest_info is False:
