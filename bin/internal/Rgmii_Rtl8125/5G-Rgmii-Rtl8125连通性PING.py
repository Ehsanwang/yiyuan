...
debug_port = 'COM'  # RM模块参数设置为''，但是需要连接跳线读取debug log，RG需要设置为DEBUG UART串口号
restart_mode = 1  # 设置是否开关机，None：每个runtimes不进行开关机，1：每个runtimes进行VBAT开关机
revision = 'RG520NEUDHR01A01M4G_OCPU_TP'  # 设置Revision版本号，填写ATI+CSUB查询的Revision部分
...
ping_times = 10  # 默认每个runtimes ping 100次
