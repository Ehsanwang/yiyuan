...
3. 手动配置开启rgmii或rtl8125模式路由拨号
    【RGMII】AT+QETH="rgmii","enable",1
            at+qmap="mpdn_rule",0,1,0,0,1
    【PCIE】at+qcfg="pcie/mode",1   at+qeth="eth_driver","r8125"/"r8168"  AT+QMAPWAC=1   [指令重启生效]
            at+qmap="mpdn_rule",0,1,0,0,1
4. Python版本大于3.8
5. 需打开adb功能
脚本逻辑:
...
dial_info = '{}_路由模式桥模式切换PING连接稳定性'.format(dial_mode)  # 脚本详情字符串
...
while True:
    ...
    # 删除规则,启用桥模式
    main_queue_data = route_queue_put('AT', 'switch_dialmode', False, runtimes)
    if main_queue_data is False:
        Rgmii_Rtl8125Manager.rgmii_rtl8125_init_fusion()
        continue
    ...
    # 删除规则,启用路由模式
    main_queue_data = route_queue_put('AT', 'switch_dialmode', True, runtimes)
    if main_queue_data is False:
        Rgmii_Rtl8125Manager.rgmii_rtl8125_init_fusion()
        continue
