...
send_data = ''  # 定义服务器所下发数据，与Qserver发送框中内容保持一致。
...
access_mode = 0    # 0：非透传Bufffer模式
runtimes = 0
connection_mode = 1  # 0代表长连,1代表短连
connection_type = "TCP"
script_start_time = time.time()  # 脚本开始的时间，用作最后log统计
tcp_info = '{}-{}-服务器主发非透传Buffer'.format(connection_type, '短连' if connection_mode == 1 else '长连')  # 脚本详情字符串
...
dial_manager = TcpUdpManager(main_queue, route_queue, uart_queue, log_queue, uart_port, version, imei, connection_mode,
                             access_mode, connection_type, server_ip, server_port, '', '', '', '', is_5g_m2_evb,
                             runtimes)
