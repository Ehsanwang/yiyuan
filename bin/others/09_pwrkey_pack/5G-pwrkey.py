...
uart_port = str(data['uart_port'])
at_port = str(data['at_port'])
dm_port = str(data['dm_port'])
debug_port = str(data['debug_port'])
revision = str(data['revision'])
sub_edition = str(data['sub_edition'])
imei = str(data['imei'])
dtr_on_time = float(data['dtr_on_time'])  # 定义POWERKEY开机脉冲时间，默认0.5S(500ms)
dtr_off_time = float(data['dtr_off_time'])  # 定义POWERKEY关机脉冲时间，默认0.8S(800ms)
