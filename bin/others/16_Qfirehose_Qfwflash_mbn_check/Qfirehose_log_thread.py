...
class LogThread(Thread):
    def __init__(self, version, info, log_queue, main_queue, is_same_upgrade):
        ...
        self.qfirehose_log_handle = open('QFirehose-{}.txt'.format(self.version), "a+", encoding='utf-8', buffering=1)
        self.handles = ['self.at_log_handle', 'self.dos_log_handle', 'self.debug_log_handle',
                        'self.result_log_handle', 'self.network_log_handle', 'self.qfirehose_log_handle']
    ...
    def qfirehose_log(self, log_queue_data):
        self.qfirehose_log_handle.write('{}{}'.format(log_queue_data, '' if log_queue_data.endswith('\n') else '\n'))
    ...
    def write_result_log(self, runtimes):
        ...
        result_width_standard_0 = {0: ['local_time', 25], 1: ['runtimes', 8], 2: ['upgrade_fail_times', 19],
                                   ...
                                   5: ['qfirehose_upgrade_time', 19]}
        # 不同版本升级
        result_width_standard_1 = {0: ['local_time', 25], 1: ['runtimes', 8], 2: ['upgrade_fail_times', 19],
                                   3: ['net_fail_times', 21], 4: ['qfirehose_version_fail_times', 19],
                                   5: ['qfirehose_upgrade_a_b_time', 19], 6: ['qfirehose_upgrade_b_a_time', 19]}
        ...
        result_list_0 = [local_time, runtimes, upgrade_fail_times, net_fail_times, qfirehose_version_fail_times, qfirehose_upgrade_time]
        result_list_1 = [local_time, runtimes, upgrade_fail_times, net_fail_times, qfirehose_version_fail_times, qfirehose_upgrade_a_b_time, qfirehose_upgrade_b_a_time]
    ...
    def end_script(self, script_start_time, runtimes):
        ...
        if self.is_same_upgrade:
            ...
            result = '\n[{}]-[{}]\n'.format(script_start_time_format, script_end_time_format) + \
                     ...
                     '升级后找网失败(net_fail_times)：{}次\n'.format(net_fail_times) + \
                     '升级版本检查失败(qfirehose_version_fail_times)：{}次\n'.format(qfirehose_version_fail_times) + \
                     'A版本升级到B版本平均时间为(fota_a_b_average_time):{}s\n'.format(fota_a_b_average_time) + \
                     'B版本升级到A版本平均时间为(fota_b_a_average_time):{}s\n'.format(fota_b_a_average_time)
        print(result)
