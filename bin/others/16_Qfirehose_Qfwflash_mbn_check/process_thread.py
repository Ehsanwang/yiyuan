...
class ProcessThread(Thread):
    ...
    def qfirehose_upgrade(self, packagename, vbat, mode, runtimes):
        self.log_queue.put(
            ['qfirehose_log', '[{}]'.format(datetime.datetime.now(), '*'*60)])
        self.log_queue.put(
            ['qfirehose_log', '[{}  {}开始进行QFirehose升级{}]'.format(datetime.datetime.now(), '*'*30, '*'*30)])
        start_time = time.time()
        random_off_time = round(random.uniform(15, 35), 2)
        upgrade = subprocess.Popen('QFirehose -f {} -p /dev/mhi_BHI'.format(packagename), stdout=PIPE, stderr=STDOUT, shell=True)
    ...
    def qfwflash_upgrade(self, packagename, vbat, runtimes):
        """
        使用Qfwflash升级工具进行单包升级，指令qfwflash -f 版本路径 -p /dev/mhi_BHI
        """
        self.log_queue.put(
            ['qfirehose_log', '[{}]'.format(datetime.datetime.now(), '*'*60)])
        self.log_queue.put(
            ['qfirehose_log', '[{}  {}开始进行单包升级{}]'.format(datetime.datetime.now(), '*'*30, '*'*30)])
        start_time = time.time()
        random_off_time = round(random.uniform(15, 35), 2)
        upgrade = subprocess.Popen('qfwflash -f {} -p /dev/mhi_BHI'.format(packagename), stdout=PIPE, stderr=STDOUT, shell=True)
        while True:
            time.sleep(0.001)
            upgrade_content = upgrade.stdout.readline().decode("UTF-8")
            if upgrade_content != '':
