...
class RestartManager:
    ...
    def init(self):
        ...
        if self.restart_mode in module_need_init_list:
            # 1.初始化拉引脚
            self.init_module()
            if self.restart_mode == 0 or self.restart_mode == 6:
                self.init_power_down()
            # 2.初始化检测USB驱动
    ...
    def powerkey(self):
        ...
        if main_queue_data is False:
            return False
        main_queue_data = self.route_queue_put('AT', 'check_power_down_urc', 'NORMAL POWER DOWN', 300, self.runtimes)
        if main_queue_data is False:
            return False
        main_queue_data = self.route_queue_put('AT', 'close', self.runtimes)
        if main_queue_data is False:
    ...
    def cfun0_1_4(self):
        """
        M.2 EVB：打开AT口->发送AT+CFUN=0->发送AT+CFUN=1->发送AT+CFUN=4->发送AT+CFUN=1关闭AT口；
        5G-EVB_V2.1：打开AT口->发送AT+CFUN=0->发送AT+CFUN=1->发送AT+CFUN=4->发送AT+CFUN=1关闭AT口。
        :return: None
        """
        # 发送 AT+CFUN=0、AT+CFUN=1和AT+CFUN=4
        main_queue_data = self.route_queue_put('AT', 'open', self.runtimes)
        if main_queue_data is False:
            return False
        ...
        if main_queue_data is False:
            return False
        time.sleep(random.uniform(10, 15))
        main_queue_data = self.route_queue_put('AT', 'AT+CFUN=4', 15, 1, self.runtimes)
        if main_queue_data is False:
            return False
        time.sleep(random.uniform(10, 15))
        main_queue_data = self.route_queue_put('AT', 'AT+CFUN=1', 15, 1, self.runtimes)
