...
class ATThread(Thread):
    ...
    def dial_assist(self, assist_phone_number, runtimes):
        self.send_at("at+colp=1", runtimes, 1)

        data = self.send_at(f"ATD{assist_phone_number};", runtimes, 30)
        if 'OK' not in data:
            self.log_queue.put(['all', '[{}] runtimes:{} 拨号失败'.format(datetime.datetime.now(), runtimes)])
            return False
        
        self.log_queue.put(['at_log', '[{}] 等待通话20S'.format(datetime.datetime.now())])
        time.sleep(20)

        self.send_at(f"ATH;", runtimes, 0.3)
