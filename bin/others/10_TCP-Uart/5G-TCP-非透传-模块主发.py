...
from tcp_log_send_buffer_push_thread import LogThread
...
5. 长连重复4-5；
6. 短连重复3-5。
默认某一传输模式进行完直接进行下一传输模式(通过修改辅助参数中的is_continue参数可配置)，
若设置连续进行，则需要设置timing值，两种传输模式各占一半时间，
"""
...
access_mode = 1    # 0:非透传 Buffer 模式, 1:非透传 Push 模式
...
while True:
    ...
    if time.time() - start_transfer_time > timing / 2 * 3600 and is_continue == 1:
        start_transfer_time = time.time()
        access_mode = 0 if access_mode == 1 else 1
        log_queue.put(['all', '[{}] runtimes{}切换传输模式为{}'.format(datetime.datetime.now(), runtimes, 'buffer' if access_mode == 0 else 'push')])
        main_queue_data = route_queue_put('AT', 'close_tcp', runtimes)
        if main_queue_data is False:
            # dial_manager.dial_init_tcp_reboot()
            continue
    ...
    # 发送接收对比数据
    main_queue_data = route_queue_put('AT', 'client_send_recv_compare', connection_mode, access_mode, runtimes)
    if main_queue_data is False:
        # dial_manager.dial_init_tcp_reboot()
        continue
