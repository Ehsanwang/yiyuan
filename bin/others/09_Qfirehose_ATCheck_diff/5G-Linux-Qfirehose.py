...
1、进行初始化;
2、检测版本号信息是否正确;
3、进行升级，可配置是否随机断电，只在相同版本升级过程中可配置随机断电;
4、升级完成后检查版本号是否正确;
5、检查注网情况;
6、循环2-5。
不同版本升级：
...
after_upgrade_revision = ""  # 设置升级后的Revision版本号，填写ATI+CSUB查询的Revision部分，如果是同版本升级则不填
after_upgrade_sub_edition = ""  # 设置升级后的SubEdition版本号，填写ATI+CSUB查询的SubEdition部分
imei = '860736040004978'  # 模块测试前配置IMEI号
VBAT = True       # 升级过程是否断电 True：升级过程中断电  False: 升级过程中不断电,目前只关注同版本断电升级
A_B_package_name = '/home/q/RG502QEAAAR01A02M4G_01.001V02.01.001V02'   # 若是同版本升级，只在此处填写当前版本包路径名称，不同版本填写升级后版本包路径名称
B_A_package_name = ''   # 若是不同版本升级，则此处填写升级前版本包名称 ,否则不填
...
Qfirehose_info = 'Qfirehose-{}-{}'.format('相同版本升级' if is_same_upgrade else '不同版本升级', '升级断电' if VBAT and is_same_upgrade else '正常升级')  # 脚本详情字符串
    ...
    if is_same_upgrade:
        ...
        for i in range(1, 5):
            ...
            if main_queue_data is False:
                ...
                continue
            if VBAT:
                qfirehose_status = qfirehose_manager.qfirehose_upgrade_vbat(A_B_package_name, runtimes)
        ...
        for mode in ('forward', 'backward'):
            if mode == 'forward':
                log_queue.put(['at_log', '[{}] 开始进行A-B升级,当前版本为{}'.format(datetime.datetime.now(), version)])
                for i in range(1, 5):
                    if i == 4:
                        log_queue.put(['all', '[{}] runtimes:{} 连续三次升级异常'.format(datetime.datetime.now(), runtimes)])
                        pause()
                    main_queue_data = route_queue_put('Process', 'qfirehose_upgrade', A_B_package_name, False, mode, runtimes)
                    if main_queue_data is False:
                        qfirehose_manager.qfirehose_normal_exception_init()
                        continue
                    # 检测驱动加载
                    ...
                    if main_queue_data is False:
                        ...
                        continue
                    # 第一次升级后check at指令
                    main_queue_data = route_queue_put('AT', 'tw_at', runtimes)
                    if main_queue_data is False:
                        qfirehose_manager.qfirehose_normal_exception_init()
                        continue
                    # 关闭AT口
                ...
                log_queue.put(['at_log', '[{}] 开始进行B-A升级,当前版本为{}'.format(datetime.datetime.now(), after_version)])
                for i in range(1, 5):
                    if i == 4:
                        log_queue.put(['all', '[{}] runtimes:{} 连续三次升级异常'.format(datetime.datetime.now(), runtimes)])
                        pause()
                    main_queue_data = route_queue_put('Process', 'qfirehose_upgrade', B_A_package_name, False, mode, runtimes)
                    if main_queue_data is False:
                        qfirehose_manager.qfirehose_normal_exception_init()
                        continue
                    # 检测驱动加载
