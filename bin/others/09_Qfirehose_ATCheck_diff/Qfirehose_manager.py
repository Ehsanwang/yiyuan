...
class Qfirehose:
    def __init__(self, main_queue, route_queue, uart_queue, log_queue, uart_port, evb, version, imei, runtimes):
    ...
    def qfirehose_init(self):
        ...
        # TODO:备份指令超时时间？
        # 6. 发送AT+QPRTPARA=1
        main_queue_data = self.route_queue_put('AT', 'AT+QPRTPARA=1', 6, 1, self.runtimes)
        if main_queue_data is False:
        ...
        if main_queue_data is False:
            ...
            exit()
        # 8. AT+QPRTPARA=4
        main_queue_data = self.route_queue_put('AT', 'AT+QPRTPARA=4', 6, 1, self.runtimes)
        if main_queue_data is False:
            print('初始化，备份指令执行异常，退出脚本')
            exit()
        # 9. 关闭AT口
    ...
    def qfirehose_normal_exception_init(self):
        # 关口防止端口变化
    ...
    def qfirehose_upgrade_vbat(self, packagename, runtimes):
        # 断电
        self.vbat()
        main_queue_data = self.route_queue_put('Uart', 'check_qfirehose_status', packagename, 'forward', runtimes)
        if main_queue_data is False:
            return False
