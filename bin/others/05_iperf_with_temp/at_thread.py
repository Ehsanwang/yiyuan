...
class ATThread(Thread):
    ...
    def dsss_init(self, runtimes):
        ...
        if simslot_index != '1':
            for i in range(1, 11):
                self.send_at('AT+QUIMSLOT=1', self.at_port_opened, runtimes, 0.3)
                simslot_return_value_re = self.send_at('AT+QUIMSLOT?', self.at_port_opened, runtimes, 0.3)
                if '1' not in simslot_return_value_re:
                    continue
            else:
                self.log_queue.put(['all', "[{}] runtimes:{} 连续10次执行at+quimslot=1失败".format(datetime.datetime.now(), runtimes)])
                input("保留现场，问题定位完成后请直接关闭脚本")
    ...
    def sms_cmgr(self, cmss_index, phone_number, runtimes):
        ...
        if sms_cmgr_value != '':
            ...
            if return_va_cmgr != '':
                if '{}'.format(phone_number) in return_va_cmgr:
            ...
            if return_cmgw_value != '':
                ...
                if '>' in return_value_cache:
                    if flag is False:
                        ...
                        self.log_queue.put(
                            ['at_log', '[{} Send] {}'.format(datetime.datetime.now(), '{}\\r\\n'.format(msg_content))])
                        return_value_cache += '[{}] {}'.format(datetime.datetime.now(), return_cmgw_value)
                        flag = True
                        time.sleep(2)
    ...
    def upl_package_at(self, file_size, package_name, package_path, runtimes):
        """
        AT方式上传FOTA差分包至UFS分区
        :param file_size: 差分包大小
        :param package_name: 差分包名称
        :param package_path: 差分包在本地存放路径
        :param runtimes:
        :return:
        """
        ...
        return_value_cache = ''
        up_flag = False
        up_file_timeout = 36000  # AT方式上传很慢，手动验证大约40分钟上传完成，超时时间给60分钟
                ...
                if re.search('CONNECT', str(return_value_cache)) and up_flag is False:
                    ...
                    with open(package_path, 'rb+') as f:
                        for i in range(ne_size // 1024 + 2):
                            ...
                            # self.log_queue.put(['at_log', '[{} Send] {}'.format(datetime.datetime.now(), '{}'.format(data_str))])
                            # self.logger.info('Send FOTA FILE: {}'.format(data_str))
                ...
                if up_flag:
                    if re.search(r'\+QFUPL: ', return_value_cache) and 'OK' in return_value_cache:
                        ...
                        if int(return_size_tostr) == file_size:
                            self.log_queue.put(['at_log', '[{}]差分包上传成功'.format(datetime.datetime.now())])
                            self.log_queue.put(['df', runtimes, 'upl_file_time', time.time() - up_file_start_time])
                            return True
                    if time.time() - up_file_start_time > up_file_timeout:
                        self.log_queue.put(['all', '[{}]runtimes: {}上传差分包超时{}S'.format(datetime.datetime.now(), runtimes, up_file_timeout)])
                        self.send_at('AT+QFDEL="{}"'.format(package_name), self.at_port_opened, 3, runtimes)
                        self.log_queue.put(['df', runtimes, 'upl_timeout_times', 1])
                        return False
