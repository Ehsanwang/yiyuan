...
class ATThread(Thread):
    ...
    def client_send_recv_compare(self, connection_mode, access_mode, runtimes):
        ...
        if access_mode == 0:    # buffer模式，不会直接返回数据本身
            ...
            while True:
                ...
                if '"recv",0,1360' in return_push_cache and '"recv",0,100' in return_push_cache:    # push模式发送成功后数据可能分为两段直接返回，一次返回1360长度内容，一次100长度内容
                    ...
                    if connection_mode == 1:    # 如果是短连需要每次断开连接
                        ...
                        time.sleep(60)  # 参考LTE确认发送成功后等待一段时间（60秒）
                    return True
                if '"recv",0,1380' in return_push_cache and '"recv",0,80' in return_push_cache:    # push模式发送成功后数据可能分为两段直接返回，一次返回1380长度内容，一次80长度内容
                    self.log_queue.put(['at_log', '[{}] 数据返回成功'.format(datetime.datetime.now())])
                    self.log_queue.put(['df', runtimes, 'push_send_end_timestamp', time.time()])
                    self.log_queue.put(['df', runtimes, 'push_send_success_times', 1])
                    if connection_mode == 1:    # 如果是短连需要每次断开连接
    ...
    def check_cfun(self, runtimes):
        for i in range(10):
            ret_val = self.send_at('AT+CFUN?', runtimes, timeout=15)
            ret_val_re = ''.join(re.findall(r'CFUN:\s(\d+)', ret_val))
            if 'OK' not in ret_val:
                continue
            if '7' in ret_val_re or '1' not in ret_val_re:
                self.log_queue.put(['all', '[{}] runtimes:{} CFUN值异常，当前CFUN：{}'.format(datetime.datetime.now(), runtimes, ret_val_re)])
                self.log_queue.put(['at_log', '[{}] 实际接收数据为{}'.format(datetime.datetime.now(), ret_val_re)])
                pause()
            return True
        else:
            self.log_queue.put(['all', '[{}] runtimes:{} 连续10次查询CFUN异常'.format(datetime.datetime.now(), runtimes)])
            pause()
