...
class LogThread(Thread):
    def __init__(self, version, info, log_queue, restart_mode, cpin_flag, main_queue):
        ...
        self.ip_result_handle = open('IPLog-{}.txt'.format(self.version), "a+", encoding='utf-8', buffering=1)
        self.handles = ['self.at_log_handle', 'self.dos_log_handle', 'self.debug_log_handle',
                        'self.result_log_handle', 'self.network_log_handle', self.ip_result_handle]
    ...
    def ip_log(self, log_queue_data):
        self.ip_result_handle.write('{}{}'.format(log_queue_data, '' if log_queue_data.endswith('\n') else '\n'))
    ...
    def write_result_log(self, runtimes):
        ...
        if self.cpin_flag:  # 如果计算锁PIN的找网时间
            check_network_time = '找网失败' if pd.isna(self.df.loc[runtimes, 'check_network_time']) else round(float(self.df.loc[runtimes, 'check_network_time']), 2)
        else:  # 如果计算不锁PIN的从开机到找网的时间
            check_network_time = '找网失败' if pd.isna(self.df.loc[runtimes, 'power_on_timestamp']) or pd.isna(self.df.loc[runtimes, 'net_register_timestamp']) else round(self.df.loc[runtimes, 'net_register_timestamp'] - self.df.loc[runtimes, 'power_on_timestamp'], 2)
        cops_value = '' if pd.isna(self.df.loc[runtimes, 'cops_value']) else self.df.loc[runtimes, 'cops_value']
        net_fail_times = int(self.df['net_fail_times'].sum())
        cfun_fail_times = int(self.df['cfun_fail_times'].sum())
        cpin_not_ready_times = int(self.df['cpin_not_ready'].sum())
