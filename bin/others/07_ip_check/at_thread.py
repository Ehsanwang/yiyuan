...
class ATThread(Thread):
    ...
    def check_ip(self, runtimes):
        cgpaddr_result = self.send_at('AT+CGPADDR', self.at_port_opened, runtimes, timeout=0.3)  # 发送AT获取值
        cgpaddr_result_re = re.findall(r'CGPADDR:\s\d+.\"(\d+.\d+.\d+.\d+),', cgpaddr_result)  # 匹配IP
        self.log_queue.put(['ip_log', "[{}] {}".format(datetime.datetime.now(), ';      '.join(cgpaddr_result_re))])
        if runtimes == 1:  # runtimes = 1的时候存储
            self.cache = cgpaddr_result_re
        elif self.cache != cgpaddr_result_re:
            self.cache = cgpaddr_result_re
            cfun_command_list = ['AT+CFUN=1', 'AT+CFUN=4']
            cfun_command = random.choice(cfun_command_list)
            self.send_at(cfun_command, self.at_port_opened, runtimes, timeout=3)  # 发送AT获取值
