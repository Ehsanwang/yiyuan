...
class ATThread(Thread):
    ...
    def prepare_at(self, version, imei, cpin_flag, runtimes):
        ...
        self.send_at('ATE1', self.at_port_opened, runtimes, 3)
        # 版本号
        ...
        if return_cmgf_re == '1':
            for i in range(1, 11):
                ...
                if '+CMGL:' not in return_cmgl and 'OK' in return_cmgl:
                    ...
                    self.send_at('AT+CMGD=0,4', self.at_port_opened, runtimes, 3)
    ...
    def check_network(self, cpin_flag, runtimes):
        ...
        while True:
            ...
            # 进行AT+COPS?找网
            cops_result = self.send_at('AT+COPS?', self.at_port_opened, runtimes, timeout=3)  # 找网设置为3S
    ...
    def qcfg_usbcfg(self, mode, runtimes):
        ...
        if mode:
            while True:
                time.sleep(1)
                return_value = self.send_at('at+qcfg="usbcfg"', self.at_port_opened, runtimes)
                if return_value != '':
                    adb_value = re.search(r'0x2C7C,0x0800,(\d,){5}(\d+)', str(return_value)).group(2)
                    if adb_value and adb_value == '1':
                        return True
                    elif adb_value == '0':
                        return_value = self.send_at('at+qcfg="usbcfg",0x2C7C,0x0800,1,1,1,1,1,1', self.at_port_opened,
                                                    runtimes, 10)
                        if 'OK' in return_value:
                            continue
            ...
            return_value = self.send_at('at+qcfg="usbcfg",0x2C7C,0x0800,1,1,1,1,1,0', self.at_port_opened, runtimes, 10)
            if 'OK' in return_value:
                return True
            else:
