...
class RestartManager:
    def __init__(self, main_queue, route_queue, uart_queue, at_queue, log_queue, at_port, uart_port, dtr_on_time, dtr_off_time, evb, restart_mode, version, imei, cpin_flag, reset_internal_time, runtimes):
        ...
        self.reset_internal_time = reset_internal_time
    ...
    def init_power_down(self):
        """
        有别于self.init初始化，此函数为仅POWERKEY(restart_mode=0)一种方式需要下面的操作
        :return:
        """
        # 删除初始化检查URC的逻辑，避免不同的开机状态引发的AT口错误
    ...
    def powerkey(self):
        ...
        main_queue_data = self.route_queue_put('Uart', 'check_message', 'Starting Power-Off', 60, self.runtimes)
        if main_queue_data is False:
            return False
    ...
    def reset(self):
        ...
        #self.log_queue.put(['at_log', "[{}] Set DTR False".format(datetime.datetime.now())])
        #self.route_queue_put('Process', 'check_usb_driver_dis', True, self.runtimes)
        time.sleep(self.reset_internal_time)
        #self.log_queue.put(['at_log', "[{}] 等待{}S后上电".format(datetime.datetime.now(), self.reset_internal_time)])
        self.route_queue_put('Uart', 'set_dtr_true')
        self.log_queue.put(['at_log', "[{}] Set DTR True".format(datetime.datetime.now())])
    ...
    def cfun1_1(self):
        ...
        if main_queue_data is False:
            return False
        # # check debuglog "Starting Reboot"
        main_queue_data = self.route_queue_put('Uart', 'check_message', 'Starting Reboot', 60, self.runtimes)
        if main_queue_data is False:
            return False
        self.log_queue.put(['df', self.runtimes, 'power_off_end_timestamp', time.time()])  # 记录关机结束时间戳
        self.route_queue_put('Process', 'check_usb_driver_dis', True, self.runtimes)
    ...
    def re_init(self):
        ...
        if self.restart_mode == 0:  # 如果POWERKEY方式出现异常，则检测当前模块状态，并尝试每次拉动RTS的时间增加0.05s，10次都开机失败，脚本暂停
            for i in range(10):
                ...
                self.log_queue.put(['at_log', "[{}] POWERKEY初始化，当前dtr_on_time：{}".format(datetime.datetime.now(), self.dtr_on_time_init)])
                if self.evb == 'M.2':
                    # 检测是否有驱动，如果有驱动，说明模块开机，需要关机
                    main_queue_data = self.route_queue_put('Process', 'check_usb_driver', False, self.runtimes)
                    if main_queue_data:  # 如果驱动在，则需要关机
                        time.sleep(30)
                        self.log_queue.put(['at_log', "[{}] 拉动DTR控制POWERKEY关机".format(datetime.datetime.now())])
                        self.route_queue_put('Uart', 'set_dtr_true')
                        self.log_queue.put(['at_log', "[{}] Set DTR True".format(datetime.datetime.now())])
                ...
                self.log_queue.put(['all', '[{}] runtimes:{} POWERKEY开机过程异常，初始化10次开机均失败'.format(datetime.datetime.now(), self.runtimes)])
                pause()
