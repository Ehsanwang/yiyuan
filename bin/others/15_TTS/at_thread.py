...
class ATThread(Thread):
    ...
    def tts_init(self, runtimes):
        """
        AT+QTTS=?返回是否正常
        AT+QTTS=0返回是否正常
        """
        # 设置为主模式
        self.send_at('AT+QDAI=x,0,0,4,0,0,1,1', runtimes, 3)
        # 测试是否支持此功能
        for j in range(1, 11):
            qtts_return_value = self.send_at('AT+QTTS=?', runtimes, 0.3)
            if '+QTTS: (0-2),<text>' in qtts_return_value:
                return True
        else:
            self.log_queue.put(['all', "[{}] runtimes:{} 连续10次执行AT+QTTS=?异常".format(datetime.datetime.now(), runtimes)])
            pause()
            ...
            while True:
                ...
                if at_returned_ok_flag:  # 如果返回OK了
                    if result in return_value_cache:
                        ...
                        self.log_queue.put(['all', '[{}] runtimes:{} {}S内未收到{}上报，请注意查看log!'.format(datetime.datetime.now(), runtimes, result_timeout, result)])
                        self.log_queue.put(['df', runtimes, 'tts_fail_times', 1])
                        return False
        ...
        while True:
            ...
            if return_value != '':
                ...
                if package_error:
                    ...
                    if vbat:
                        self.log_queue.put(['at_log', '[{}] 升级上报END 0后断电'.format(datetime.datetime.now())])  # 写入log
                        return True
                    ...
                    if time.time() - start_time > vbat_point:
                        ...
                        if vbat_point == 2:
                            time.sleep(1)  # JIRA STSDX55-2067
                        return True
    ...
    def tts_send(self, runtimes):
        """
        测试AT+QTTS=2,“”。
        :param runtimes: 当前脚本的运行次数
        :return: True: 正常切换；False：切换过程中发现异常。
        """
        tts_result_status = True
        little_times = 0
        for i in range(1, 10):
            little_times = little_times + 1
            value = 'hello' + str(little_times)
            tts_result_status = self.send_at_two_steps_check('AT+QTTS=2,"{}"'.format(value), runtimes, 3, '+QTTS: 0', 10)
            self.log_queue.put(['df', runtimes, 'tts_send_times', little_times])
            time.sleep(8)
            if tts_result_status is False:
                return False
            else:
                continue
        little_times = little_times + 1
        value = 'hello' + str(little_times)
        self.send_at('AT+QTTS=2,"{}"'.format(value), runtimes, timeout=0.3)
        tts_result_status2 = self.send_at_two_steps_check('AT+QTTS=0', runtimes, 3, '+QTTS: 0', 10)
        self.log_queue.put(['df', runtimes, 'tts_send_times', little_times])
        if tts_result_status2 is False:
            return False
        else:
            self.log_queue.put(['df', runtimes, 'tts_fail_times', 0])
            return True
    ...
    def upl_package_at(self, package_name, package_path, runtimes):
        """
        AT方式上传文件至UFS分区
    ...
    def ufs_updw_file(self, package_name, package_path, qfopen_mode, runtimes):
        """
        :param package_name:
        :param package_path:
        :param qfopen_mode:
        ...
        # 上传文件
        upl_return_value = self.upl_package_at(package_name, package_path, runtimes)
        if upl_return_value:
    ...
    def efs_check(self, qftest_filesize, qftest_file_num, qftest_test_cnt, qftest_write_interval, runtimes):
        efs_test_cmd = f'AT+QFTEST="EFS/StressWrite",' \
                       f'1,' \
                       f'{qftest_filesize},' \
                       f'{qftest_file_num},' \
                       f'{qftest_test_cnt},' \
        ...
        while True:  # 进行： Ok:0x0,1,3,0,10，# 结束： Ok:0x0,0,10,0,0
            ...
            efs_status = self.send_at('AT+QFTEST="EFS/StressWrite",2', runtimes, timeout=5)
            ...
            block_status = self.send_at('AT+QFTEST="EFS/BlockEC"', runtimes, timeout=5)
