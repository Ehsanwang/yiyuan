...
class LogThread(Thread):
    ...
    def init_log_dir(self, info):
        ...
        init_df_column = ['runtimes_start_timestamp', 'runtimes', 'tts_send_times'
                          'tts_fail_times', 'qftest_record']
    ...
    def write_result_log(self, runtimes):
        ...
        result_width_standard_1 = {0: ['local_time', 25], 1: ['runtimes', 8],
                                   2: ['tts_send_time', 11], 3: ['tts_fail_times', 15]}
        ...
        tts_send_times = int(self.df['tts_send_times'].sum())
        tts_fail_times = int(self.df['tts_fail_times'].sum())

        result_list = [local_time, runtimes, tts_send_times, tts_fail_times]
    ...
    def end_script(self, script_start_time, runtimes):
        ...
        # 统计参数
        tts_send_times: int = int(self.df['tts_send_times'].sum())
        tts_fail_times: int = int(self.df['tts_fail_times'].sum())
        result_all_2 = '发送语音次数(tts_send_times)：{}次\n'.format(tts_send_times) + \
                           '发送语音后返回异常次数(tts_fail_times): {}次\n'.format(tts_fail_times)
        print(('\n-----------压力统计结果start-----------{}-----------压力统计结果end-----------\n'.format(result_all_1 + result_all_2)))
